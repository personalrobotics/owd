  ...
  last_locked_thread_id(0)
{
  ...
  links[Link::L1]=Link( DH( M_PI_2,   0.0000,   0.0000,   0.0000), 1.072,
			R3(0.00033, -0.2219, -0.00167),
			Inertia(0.0574324058, 0, 0,
                                0.0020376664, 0, 0.0559620908) );
  links[Link::L2]=Link( DH(-M_PI_2,   0.0000,   0.0000,   0.0000), 2.846,
			R3( 0.0147, 0.0166, 0.04308),
			Inertia(0.0082126968, 0, 0,
                                0.0041531644, 0, 0.0074171174) );
...
#else // HEAD
  //motors[1].ID=1; motors[1].puckI_per_Nm = 14524; //Maxon EC90 70.5 mNm/A and 1024c/A

  motors[1].ID=1; motors[1].puckI_per_Nm = 24154; // Faulhaber motor 41.4 mNm/A and 1024c/A
  motors[2].ID=2; motors[2].puckI_per_Nm = 27750; // 36.9 mNm/A and 1024c/A
#endif // HEAD
...
#else // HEAD
  Joint::MAX_MECHANICAL_TORQ[0] = 1.0 * Puck::MAX_TRQ[0] / motors[1].puckI_per_Nm * mN1;
  Joint::MAX_MECHANICAL_TORQ[1] = 1.0 * Puck::MAX_TRQ[1] / motors[2].puckI_per_Nm * mN2;
#endif // HEAD
...
#else // HEAD
  Puck::MAX_CLIPPABLE_TRQ[0] = (Joint::MAX_MECHANICAL_TORQ[0] / mN1) * motors[1].puckI_per_Nm;
  Puck::MAX_CLIPPABLE_TRQ[1] = (Joint::MAX_MECHANICAL_TORQ[1] / mN2) * motors[2].puckI_per_Nm;
#endif // HEAD
  /* Print result */
  ss.str("");
...
int WAM::send_mtrq(){
  ...
  for(int m=Motor::M1; m<=Motor::Mn; m++) {
    try {
      mtrq[ motors[m].id() ] = 
	(motors[m].t + motors[m].cogging_comp_torque()) * motors[m].IPNm();
    } catch (...) {
      // error with cogging comp torque, so leave it out
      mtrq[ motors[m].id() ] = motors[m].t * motors[m].IPNm();
    }
  }
...
#else // HEAD
  // JOINT 1
  joints[1].motor_position =  -motors[1].q/mN1;
  joints[1].encoder_position = bus->jpos[1];

  if (use_joint_encoders && 
      (joints[1].encoder_position < 100)) {
    joints[1].q = joints[1].encoder_position + joints[1].offset;
  } else {
    joints[1].q = joints[1].motor_position + joints[1].offset;
  }

  // MOTOR 2
  joints[2].motor_position =  motors[2].q/mN2;
  joints[2].encoder_position = -bus->jpos[2];

  if (use_joint_encoders &&
      (joints[2].encoder_position < 100)) {
    joints[2].q = joints[2].encoder_position + joints[2].offset;
  } else {
    joints[2].q = joints[2].motor_position + joints[2].offset;
  }
#endif // HEAD
}
 ...
 * CANBus::send_positions(double*) fails so that we can write them back...
 */
...
void WAM::get_current_data(double* pos, double *trq, double *nettrq, double *simtrq,double *trajtrq){
    ...
    this->unlock("WAM_L824");
}

void WAM::get_joint_encoder_positions(double* jpos) {
  if (jpos){
    this->lock("WAM_L829");
#ifndef HEAD
    for(int j=1; j<=4; ++j) {
    ...
    for(int j=1; j<=2; ++j) {
#endif // HEAD
      jpos[ joints[j].id() ] = joints[j].encoder_position;
    }
    this->unlock("WAM_L837");
  }
}

void WAM::get_motor_positions(double* mpos) {
  if (mpos){
    this->lock("WAM_L843");
    for(int j=Joint::J1; j<=Joint::Jn; j++)
      mpos[ joints[j].id() ] = joints[j].motor_position;
  }
  this->unlock("WAM_L847");
}
...
#define ARRAY_SIZE 30000
  ...
  while((ctrl_loop->state_rt() == CONTROLLOOP_RUN)
...
#ifdef WRIST // 7-dof
	int all_pucks = 0xFE;
#elif defined HEAD  // 2-dof
	int all_pucks = 0x06;
#else // 4-dof
	int all_pucks = 0x1E;
#endif
	if (!torques_sent && ((wam->bus->received_position_flags & all_pucks) == all_pucks)) {
	...
	// time expired
          if(locked){
              wam->unlock("WAM_L1171");
              locked = false;
          }
	++total_missed_data_cycles;
	if (++missing_data_cycles == 100) {
	  // we went 100 cycles in a row while missing values from at
	  // least 1 puck
	  ROS_FATAL("Missed CANbus replies from 100 cycles in a row");
...
#else
  static double safety_torqs[2]={3.0, 3.0};
#endif
...
WAM::~WAM() {
  ...
  if (bus) {
#ifdef HEAD
    // shut down the pucks, since there's no safety puck to do it for us
    bus->set_property_rt(GROUPID(4),MODE,0,false);
#endif
    delete bus;
    bus=NULL;
  }
