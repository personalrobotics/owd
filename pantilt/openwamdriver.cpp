  ...
  modified_j1(false)
{
...
#else // HEAD
  Plugin::_max_joint_vel.resize(2);
  Plugin::_joint_vel.resize(2);
  Plugin::_joint_accel.resize(2);
  Plugin::_max_joint_vel[0]=2; // J1
  Plugin::_max_joint_vel[1]=2; // J2
  Plugin::_joint_vel[0]=1; // J1
  Plugin::_joint_vel[1]=1; // J2
#endif // HEAD
  ...
  wamstate.target_positions.resize(7,0.0f);
  wamstate.velocities.resize(7,0.0f);
  wamstate.torques.resize(7,0.0f);
  //waminternals.motor_positions.resize(7,0.0f);
  //waminternals.joint_positions.clear();
  ...
  wamstate.target_positions.resize(nJoints,0.0f);
  wamstate.velocities.resize(nJoints,0.0f);
  wamstate.torques.resize(nJoints,0.0f);

  //waminternals.motor_positions.resize(nJoints,0.0f);
  //waminternals.joint_positions.clear();
...
#else // HEAD
  wam_tf_base[1] = tf::Transform(HALFPI_ROLL);
  wam_tf_base[2] = tf::Transform(NEG_HALFPI_ROLL);
#endif // HEAD
}
...
bool WamDriver::Init(const char *joint_cal_file)
{
...
#else // HEAD
  Plugin::_lower_jlimit.resize(2);
  Plugin::_upper_jlimit.resize(2);
  double lj[2]= {-2.12, -1.37};
  memcpy(&Plugin::_lower_jlimit[0],lj,2*sizeof(double));
  double uj[2]= { 2.67,  1.37};
  memcpy(&Plugin::_upper_jlimit[0],uj,2*sizeof(double));
#endif  
  ...
  if (bus->simulation) {
    ...
    // retrieve the motor offsets from the pucks so that we know exactly 
    // where the rotors are for compensation of cogging torque

    for (int puckid=1; puckid<=Motor::Mn; ++puckid) {
      get_puck_offset(puckid, NULL, NULL);
    }

  // read in any cogging compensation files that have been defined
  // (assumes the defined motors are in the range 1-2*nJoints)
  for (unsigned int i=1; i<=2*nJoints; ++i) {
    std::string cogfile;
    char pname[50];
    snprintf(pname,50,"motor%d_cogging_comp_file",i);
    if (n.getParam(std::string(pname), cogfile)) {
      for (int j=0; j<Motor::Mn; ++j) {
	if (owam->motors[j].ID == (int)i) {
	  if (! owam->motors[j].load_cogging_data(cogfile.c_str())) {
	    ROS_WARN("Could not load cogging compensation data for motor %d",
		     owam->motors[j].ID);
	  } else {
	    ROS_INFO("Loaded cogging compensation data for motor %d from %s",
		     owam->motors[j].ID, cogfile.c_str());
	  }
	  break;
	}
      }
...
int WamDriver::get_puck_offset(int puckid, int32_t *mechout, int32_t *apout) {
    ...
    owam->motors[puckid].offset = p-mech;
    return p-mech;
}
...
void WamDriver::set_home_position() {
  ...
  free(line);
    ...
    int32_t mpos, jpos;
    do {
      if (bus->get_property_rt(1, AP, &mpos, 2000, 1, &jpos) != OW_SUCCESS) {
	ROS_FATAL("Unable to retrieve JP from puck 1");
	throw -1;
      }
      usleep(2000);
    } while ((jpos < (1<<20)) && (jpos > -(1<<20)));
    ROS_WARN("Got it.");

    ROS_WARN("Now please tilt the head to the extreme upper angle to calibrate the TILT axis");
    do {
      if (bus->get_property_rt(2, AP, &mpos, 2000, 1, &jpos) != OW_SUCCESS) {
	ROS_FATAL("Unable to retrieve JP from puck 2");
	throw -1;
      }
      usleep(2000);
    } while ((jpos < (1<<20)) && (jpos > -(1<<20)));
    ROS_WARN("Got it. Setting motor encoders and starting controllers.");

    // Get the encoder positions again and let the CANbus class turn them
    // into joint values
    if(bus->request_positions_rt(GROUPID(4)) == OW_FAILURE){
      ROS_FATAL("set_home_position: request_positions failed");
      throw -1;
    }
    for (int i=0; i<2; ++i) {
      uint8_t  msg[8];
      int32_t msgid, msglen;
      if (bus->read_rt(&msgid, msg, &msglen, 2000) == OW_FAILURE){
	ROS_FATAL("Did not receive a response to request_positions");
	throw -1;
      }
	...
	// 22-bit AP response
	if (bus->process_positions_rt(msgid, msg, msglen) != OW_SUCCESS) {
	  ROS_FATAL("Error processing position response");
	  throw -1;
	}
      }
    }
    // now set the motor encoders to yield the same joint angle as the
    // joint encoders
    double jpositions[Joint::Jn+1];
    bus->read_joint_encoder_positions(jpositions); // will use the joint encoder values
    if (owam->set_jpos(jpositions)!= OW_SUCCESS) {
      ROS_FATAL("Error while setting motor encoder values");
      throw -1;
    }
    bus->firstupdate[1]=true;
    bus->firstupdate[2]=true;
    ROS_DEBUG("J1 joint encoder is now %2.6f, motor encoder set to %2.6f",
	      jpositions[1],owam->joints[1].q);
    ROS_DEBUG("J2 joint encoder is now %2.6f, motor encoder set to %2.6f",
	      jpositions[2],owam->joints[2].q);
    
    // switch the control loop over to using the joint encoder values
    owam->use_joint_encoders=true;
#endif // HEAD
}
...
#if !defined(BH280_ONLY) && !defined(HEAD)
...
#endif // !BH280_ONLY && !HEAD
#if defined HEAD
    if (bus->set_property_rt(GROUPID(4),MODE,2,false) == OW_FAILURE) {
      ROS_FATAL("Error changing mode of pucks");
      throw -1;
    }
#endif // HEAD
}
...
bool WamDriver::move_until_stop(int joint, double stop, double limit, double velocity, double &original_joint_pos) {
    ...
    owam->lock("owd_L1768"); // lock before checking jointstraj
    while (owam->jointstraj) {
      if (owam->jointstraj->state() == OWD::Trajectory::STOP) {
            owam->unlock("owd_L1771");
            // joint is stuck, so stop the trajectory
            ROS_DEBUG_NAMED("calibration","Joint hit torque limit.");
            owam->cancel_trajectory();
            owam->lock("owd_L1775"); // just so that we can unlock again outside the loop
            break;
        }
        owam->unlock("owd_L1778");
        usleep(50000); // 50ms delay
        owam->lock("owd_L1780"); // lock before rechecking jointstraj
    }
    owam->unlock("owd_L1782");
...
bool WamDriver::move_joint(int joint, double newpos, double velocity) {
    ...
    // loop until the trajectory finishes or gets stuck
    owam->lock("owd_L1828"); // lock before checking jointstraj
    while (owam->jointstraj) {
      if (owam->jointstraj->state()==OWD::Trajectory::STOP) {
            owam->unlock("owd_L1831");
            // joint got stuck
            owam->cancel_trajectory();
            return false;
        }
        owam->unlock("owd_L1836");
        usleep(50000); // 50ms delay
        owam->lock("owd_L1838"); // lock before rechecking jointstraj
    }
    owam->unlock("owd_L1840");
...
bool WamDriver::Publish() {
  double jointpos[nJoints+1];
  double motorpos[nJoints+1];
  double joint_encoder_pos[4+1];
  ...
  owam->get_joint_encoder_positions(joint_encoder_pos);
  owam->get_motor_positions(motorpos);
  owam->get_gains(waminternals.gains);

  for (unsigned int i=0; i<nJoints; ++i) {
    wamstate.positions[i] = jointpos[i+1];
    wamstate.target_positions[i] = owam->last_control_position[i];
    //waminternals.motor_positions[i] = motorpos[i+1];
  ...
  bool valid_jpos=true;
#ifndef HEAD
  const int NUM_JOINT_ENCODERS=4;
#else
  const int NUM_JOINT_ENCODERS=2;
#endif
  for (int i=0; i<NUM_JOINT_ENCODERS; ++i) {
    if (joint_encoder_pos[i+1] > 99) {
      valid_jpos=false;
      break;
    }
  }
  ...
  /*
  if (valid_jpos) {
    waminternals.joint_positions.resize(NUM_JOINT_ENCODERS);
    for (int i=0; i<NUM_JOINT_ENCODERS; ++i) {
      waminternals.joint_positions[i]=joint_encoder_pos[i+1];
    }
  } else {
    waminternals.joint_positions.clear();
  }
  ...
  if (!owam->jointstraj) {
    ...
    if (trajectory_list.size() > 0) {
      ...
      if (! owam->jointstraj) {
        // the trajectory finished while we were monkeying around.
        // go back and try again.
        owam->unlock("owd_L2419");
        return AddTrajectory(traj,failure_reason);
      }
      curpoint = owam->jointstraj->end_position;
      owam->unlock("owd_L2423");
    }
    // make sure first point matches current pos or last queued pos.
...
void WamDriver::Update() {
  ...
  if (++statcount == 20) {
    ...
    if (owam->recorder.count > 1000) {
      ...
      char filename[200];
      snprintf(filename,200,"~/.ros/log/wamstats%02d-%04d.csv",bus->id,dumpnum++);
