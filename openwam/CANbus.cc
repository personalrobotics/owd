...
CANbus::CANbus(int32_t bus_id, int number_of_arm_pucks, bool bh280, bool ft, bool tactile, bool log_cb_data) : 
  ...
  pos(NULL), rawpos(NULL), jpos(NULL), forcetorque_data(NULL), 
  ...
  last_encoder_clocktime(15)
{
  ...
  for(int p=1; p<=n_arm_pucks; p++){
    pos[p] = 0.0;
    trq[p] = 0;
    jpos[p] = 10; // flag for being unset
    pucks[p].ID = p;
    pucks[p].motor_id = p;
    if (p<5) { // 4-DOF
      pucks[p].group_id = 1;
    ...
    pucks[p].cpr = 4096;
#ifdef HEAD
    pucks[p].j_cpr = 8192*40;
#endif
    jumptime[p]=0;
    firstupdate[p]=true;
  }
#ifndef HEAD
  pucks[1].j_cpr = 1600000; // approximate
  pucks[2].j_cpr = 16384*40;
  pucks[3].j_cpr = 16384*40;
  pucks[4].j_cpr = 8192*40;
#endif
...
int CANbus::get_property_rt(int32_t nid, int32_t property, int32_t* value, int32_t usecs, int32_t retries, int32_t *value2){
  ...
  REREAD:
    ...
    // Parse the reply
    if(parse(msgid, msg, msglen, &nodeid, &prop, value, value2) == OW_FAILURE){
...
int CANbus::read_motor_positions(double* positions){
...
int CANbus::read_joint_encoder_positions(double *positions) {
  for (int p=1; p<=n_arm_pucks; p++) {
    positions[p] = jpos[p];
  }
  return OW_SUCCESS;
...
int CANbus::process_positions_rt(int32_t msgid, uint8_t* msg, int32_t msglen) {
  ...
  if ((nodeid >= 1) && (nodeid <=n_arm_pucks)) {
    ...
    if (msglen==6) {
      ...
      // Once a puck has seen the index pulse for a particular joint encoder,
      // the puck will output 22-bit values using -(1^21) as the zero.  In this
      // manner we can instantly tell at any time whether the joint encoder has
      // been calibrated by seeing the index pulse.  
      if (value2 > (1<<20)) {
	// negative working range
	jpos[pucks[nodeid].motor() ] = 2.0*M_PI*( (double) (value2-(1<<21)))/ 
	  ( (double) pucks[nodeid].J_CPR() );
      } else if (value2 < -(1<<20)) {
	// positive working range
	jpos[pucks[nodeid].motor() ] = 2.0*M_PI*( (double) (value2+(1<<21)))/ 
	  ( (double) pucks[nodeid].J_CPR() );
      }
      // otherwise, don't use the encoder value at all because we haven't
      // seen the index pulse yet.
    }
