...
private:
  ...
  //for lowpass filter for derivative:
  double last_ed; //the last value of the filtered derivative 
  double alpha;   //filter parameter: alpha 0:1, (no transmission):(no filtering)

  double last_e;     // Last error
  int last_e_valid;  // Last error valid? 
...
public:
  ...
  JointCtrlPID(double _kp, double _kd, double _ki, double _alpha)
  {
    Kp    = _kp;
    Kd    = _kd;
    Ki    = _ki;
  ...
  double evaluate(double qs, double q, double dt)
  {
    if(state() == Controller::RUN){
      //      lock();
      double e = qs - q;
      double ed_current  = last_e_valid ? (e - last_e)/dt : 0.0;
      double ed_filtered = last_ed + (alpha * (ed_current - last_ed));
      ...
      //      unlock();
      return Kp*e + Kd*ed_filtered + Ki*se;
    }
    return 0.0;
  }
