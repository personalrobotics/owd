...
#else // HEAD
  //filter parameter: alpha 0:1, (no transmission):(no filtering) 
  //jcontrollers.push_back(JointCtrlPID(.5, .06, .005, .4)); // pan gains, with derivative filtering active
  jcontrollers.push_back(JointCtrlPID(2.25, .08, .01)); // pan gains
  jcontrollers.push_back(JointCtrlPID(4, .15, 0.02));       // tilt gains
#endif // HEAD
}
